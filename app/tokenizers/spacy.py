    ...
    def initialize(self, **kwargs) -> None:
        ...
        Args:
            **kwargs: Initialization options:
                ...
                - disable: Components to disable (default: ['ner', 'attribute_ruler'])
        """
        try:
            import spacy
            ...
            try:
                ...
                logger.warning(f"Could not load spaCy model {model}: {e}")
                logger.warning(f"Creating blank model with sentencizer for {model}")
            ...
            logger.info(f"spaCy tokenizer initialized in {self._initialization_time:.2f}s")
            
        except ImportError as e:
            logger.error(f"spaCy import error: {str(e)}")
            raise RuntimeError(f"Could not import spaCy: {str(e)}. Please install with 'pip install spacy'.")
